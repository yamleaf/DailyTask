...
其他 OS 需要自行测试——理论上能安装就能用，但不保证兼容性，需注意！
...
本文档对应的仓库 **`yamleaf/DailyTask`** 为 **AndroidCoderPeng/DailyTask** 的分支（Fork）版本。
若您需要寻找原作者维护的原版项目，请前往 [**_原仓库_**](https://github.com/AndroidCoderPeng/DailyTask)。

> [!CAUTION]
> 本分支在保留原版全部功能的基础上做了大量增强与重构（见下文「相比原仓库的优化」），
> 改动较大、发布节奏独立，**不能简单回退到原版**；请按本文档介绍的功能为准。

### 相比原仓库的优化

> 原版具备的功能均完整保留并优化增强。本分支相较原仓库主要新增/重构了以下能力：

| 类别 | 说明 |
|:---|:---|
| **远程控制端** | 新增配套控制端 [DailyController](https://github.com/yamleaf/DailyController)，扫码配对即可远程控制、查看状态快照、下发设置 |
| **MQTT 通信协议** | 独立抽取共享协议库 [DailyProtocol](https://github.com/yamleaf/DailyProtocol)（git 子模块），HMAC-SHA256 验签 + AES-256-GCM 信封加密 |
| **告警体系** | 低电量分段告警与智能预警；设备 ID 冲突自动仲裁上报；控制端离线期间告警自动回放补收 |
| **后台保活** | 息屏三档保活（AUTO / ALARM / CPU）；进程被杀由闹钟兜底复活（计划点后 120s 内补执行）；覆盖安装自动恢复前台服务与监听 |
| **任务调度** | 调度器全面重构：打卡前预热闹钟提前唤醒、宽限期自动补执行、统一时间校验防漏打/重打、节假日自动跳过 |
| **屏幕管理** | 屏幕模式三档（伪息屏 / 息屏 / 常亮）；无障碍服务辅助截屏与文本读取 |
| **数据安全** | 消息渠道凭据加密存储；配置加密导入/导出（跨设备迁移）；交互报文多种校验模式 |
| **界面重构** | 全新配色语言：深色模式适配、统一弹窗体系、磨砂悬浮导航与状态组件全面翻新 |
...
> 否则请加入 QQ 群获取安装包。
>
> **Alpha 优化版下载**：可在[Actions](https://github.com/yamleaf/DailyTask/actions) 的工件（Artifacts）页面或
> [Releases](https://github.com/yamleaf/DailyTask/releases) 的 `DailyTask Alpha`
> 中下载 release APK，解压密码可从QQ群中询问。Alpha 版仅用于个人学习使用，不保证稳定性，使用前请进行充分验证，避免造成不必要损失。
...
本分支在保留原版全部功能的基础上进行了增强与重构，详细的优化对比请见文首「[关于本仓库](#关于本仓库)」中的
**相比原仓库的优化**一览（远程控制端 / MQTT 协议 / 告警体系 / 后台保活 / 任务调度 / 屏幕管理 / 数据安全 / 界面重构）。
...
> [!WARNING]
> 以下情形风险较高，可能被考勤系统判定为作弊，请谨慎使用：
>
> - **极高**：手机已 Root · 模拟定位修改打卡位置 · 使用向日葵等远程控制软件 · 使用 ADB 命令模拟手指点击打卡
> - **中等**：开启无障碍服务 · 手机通过数据线连接电脑
