...
| 序号 | 类别 | 内容 |
|:--:|:---|:---|
| 1  | **远程控制** | 新增远程控制：配合控制端 [DailyController](https://github.com/yamleaf/DailyController) 扫码配对，支持远程控制设备、查看状态快照、下发设置 |
| 2  | **告警体系** | 新增低电量分段告警与智能预警；设备 ID 冲突自动仲裁并上报；控制端离线期间告警自动回放补收 |
| 3  | **后台保活** | 新增息屏三种保活模式 AUTO / ALARM / CPU；进程被杀后由闹钟兜底复活（计划点后 120 秒内仍可补执行）；覆盖安装后自动恢复前台服务、闹钟与通知监听绑定 |
| 4  | **屏幕管理** | 屏幕模式三档可选：伪息屏（延时后自动全黑蒙层）/ 息屏 / 常亮；新增无障碍服务辅助截屏与文本读取 |
| 5  | **桌面组件** | 新增桌面宠物与闲置状态球悬浮组件 |
| 6  | **数据安全** | 消息渠道凭据加密存储；配置加密导出/导入（跨设备迁移）；交互报文多种校验模式 |
| 7  | **任务调度** | 调度器全面重构：新增打卡前预热闹钟提前唤醒设备；进程被杀或系统节流导致错过计划点时，仍在宽限期内自动补执行，统一时间校验防漏打/重打 |
| 8  | **界面重构** | 使用全新的配色语言对界面进行整体重构：深色模式适配、统一弹窗体系、磨砂悬浮导航与状态组件样式全面翻新 |
